...
function f(t2) {
  ...
  if (i2 >= e2)
    t2.err = `${c}: ${h} at index ${i2}, "pathValue" is missing param`;
  else if (s2 = r2.charCodeAt(i2), 43 !== s2 && 45 !== s2 || (i2 += 1, s2 = r2.charCodeAt(i2)), l(s2) || 46 === s2) {
    if (46 !== s2) {
      if (a2 = 48 === s2, i2 += 1, s2 = r2.charCodeAt(i2), a2 && i2 < e2 && s2 && l(s2))
        return void (t2.err = `${c}: ${h} at index ${n2}, "${r2[n2]}" illegal number`);
      for (; i2 < e2 && l(r2.charCodeAt(i2)); )
        i2 += 1, o2 = true;
      s2 = r2.charCodeAt(i2);
    }
    if (46 === s2) {
      for (u2 = true, i2 += 1; l(r2.charCodeAt(i2)); )
        i2 += 1, m2 = true;
      s2 = r2.charCodeAt(i2);
    }
    if (101 === s2 || 69 === s2) {
      if (u2 && !o2 && !m2)
        return void (t2.err = `${c}: ${h} at index ${i2}, "${r2[i2]}" invalid float exponent`);
      if (i2 += 1, s2 = r2.charCodeAt(i2), 43 !== s2 && 45 !== s2 || (i2 += 1), !(i2 < e2 && l(r2.charCodeAt(i2))))
        return void (t2.err = `${c}: ${h} at index ${i2}, "${r2[i2]}" invalid integer exponent`);
      for (; i2 < e2 && l(r2.charCodeAt(i2)); )
        i2 += 1;
    }
    t2.index = i2, t2.param = +t2.pathValue.slice(n2, i2);
  } else
    t2.err = `${c}: ${h} at index ${i2}, "${r2[i2]}" is not a number`;
}
function y(t2) {
  const { pathValue: e2, max: r2 } = t2;
  for (; t2.index < r2 && (10 === (n2 = e2.charCodeAt(t2.index)) || 13 === n2 || 8232 === n2 || 8233 === n2 || 32 === n2 || 9 === n2 || 11 === n2 || 12 === n2 || 160 === n2 || n2 >= 5760 && [5760, 6158, 8192, 8193, 8194, 8195, 8196, 8197, 8198, 8199, 8200, 8201, 8202, 8239, 8287, 12288, 65279].includes(n2)); )
    t2.index += 1;
  var n2;
}
function x(t2) {
...
function p(t2) {
  ...
  if (t2.segmentStart = n2, function(t3) {
    ...
    t2.err = `${c}: ${h} "${r2[n2]}" is not a path command`;
}
function g(t2) {
...
function M(t2) {
  if (b(t2))
    return g(t2);
  const e2 = new d(t2);
  for (y(e2); e2.index < e2.max && !e2.err.length; )
    p(e2);
...
function v(t2) {
  ...
  return e2.map((t3) => {
    ...
    if ("M" === a2)
      return [r2, n2] = e3, s2 = r2, i2 = n2, ["M", r2, n2];
    let m2 = [];
    if (a2 !== o2)
      switch (o2) {
        case "A":
...
function N(t2) {
  ...
  return e2.map((t3) => {
    ...
    if (a2 !== o2)
      ...
      "m" === a2 && (s2 = e3[0] + r2, i2 = e3[1] + n2), m2 = [o2, ...e3];
    const c2 = m2.length;
    switch (o2) {
      case "z":
        r2 = s2, n2 = i2;
...
function C(t2, e2, r2) {
  if (t2[r2].length > 7) {
    ...
    let s2 = r2;
    for (; n2.length; )
      e2[r2] = "A", t2.splice(s2 += 1, 0, ["C", ...n2.splice(0, 6)]);
    t2.splice(r2, 1);
  }
}
function P(t2) {
...
function k(t2, e2) {
  ...
  let m2 = t2;
  if ("TQ".includes(r2) || (e2.qx = null, e2.qy = null), "H" === r2)
    m2 = ["L", t2[1], s2];
  else if ("V" === r2)
    m2 = ["L", n2, t2[1]];
  else if ("S" === r2) {
...
function q(t2, e2, r2, n2, s2, i2, a2, o2, m2, c2) {
  ...
  if (j2[0] = 2 * V2[0] - j2[0], j2[1] = 2 * V2[1] - j2[1], c2)
    return [...j2, ...L2, ...I2, ...A2];
  A2 = [...j2, ...L2, ...I2, ...A2];
  const z2 = [];
  for (let t3 = 0, e3 = A2.length; t3 < e3; t3 += 1)
    z2[t3] = t3 % 2 ? E(A2[t3 - 1], A2[t3], g2).y : E(A2[t3], A2[t3 + 1], g2).x;
  return z2;
}
...
function D(t2, e2, r2, n2) {
  ...
  if ("TQ".includes(s2) || (r2.qx = null, r2.qy = null), ["V", "H", "S", "T", "Z"].includes(s2))
    y2 = [s2, ...a2];
  else if ("L" === s2)
    i2(h2) === i2(x2) ? y2 = ["V", p2] : i2(f2) === i2(p2) && (y2 = ["H", x2]);
  else if ("C" === s2) {
    const [t3, e3] = o2;
...
function Y(t2) {
  ...
  if (!r2.every((t3) => !Number.isNaN(t3)))
    throw TypeError(`CSSMatrix: "${t2}" must only have numbers.`);
  if (16 === r2.length) {
...
function B(t2) {
  ...
  if ("object" == typeof t2 && e2.every((e3) => e3 in t2))
    return Y([t2.m11, t2.m12, t2.m13, t2.m14, t2.m21, t2.m22, t2.m23, t2.m24, t2.m31, t2.m32, t2.m33, t2.m34, t2.m41, t2.m42, t2.m43, t2.m44]);
  throw TypeError(`CSSMatrix: "${JSON.stringify(t2)}" is not a DOMMatrix / CSSMatrix / JSON compatible object.`);
}
function J(t2) {
  if ("string" != typeof t2)
    throw TypeError(`CSSMatrix: "${t2}" is not a string.`);
  ...
  return e2.split(")").filter((t3) => t3).forEach((t3) => {
    ...
    if ("perspective" === e3 && a2 && [o2, m2].every((t4) => void 0 === t4))
      r2.m34 = -1 / a2;
    else if (e3.includes("matrix") && [6, 16].includes(i2.length) && i2.every((t4) => !Number.isNaN(+t4))) {
      const t4 = i2.map((t5) => Math.abs(t5) < 1e-6 ? 0 : t5);
      r2 = r2.multiply(Y(t4));
    } else if ("translate3d" === e3 && u2.every((t4) => !Number.isNaN(+t4)))
      r2 = r2.translate(a2, o2, m2);
    else if ("translate" === e3 && a2 && void 0 === m2)
      r2 = r2.translate(a2, o2 || 0, 0);
    else if ("rotate3d" === e3 && l2.every((t4) => !Number.isNaN(+t4)) && c2)
      r2 = r2.rotateAxisAngle(a2, o2, m2, c2);
    else if ("rotate" === e3 && a2 && [o2, m2].every((t4) => void 0 === t4))
      r2 = r2.rotate(0, 0, a2);
    else if ("scale3d" === e3 && u2.every((t4) => !Number.isNaN(+t4)) && u2.some((t4) => 1 !== t4))
      r2 = r2.scale(a2, o2, m2);
    else if ("scale" !== e3 || Number.isNaN(a2) || 1 === a2 || void 0 !== m2)
      if ("skew" === e3 && (a2 || !Number.isNaN(a2) && o2) && void 0 === m2)
        r2 = r2.skew(a2, o2 || 0);
      else {
        if (!(/[XYZ]/.test(e3) && a2 && [o2, m2].every((t4) => void 0 === t4) && ["translate", "rotate", "scale", "skew"].some((t4) => e3.includes(t4))))
          throw TypeError(n2);
        if (["skewX", "skewY"].includes(e3))
          r2 = r2[e3](a2);
        else {
...
var st = class {
  ...
  setMatrixValue(t2) {
    return "string" == typeof t2 && t2.length && "none" !== t2 ? J(t2) : [Array, Float64Array, Float32Array].some((e2) => t2 instanceof e2) ? Y(t2) : [st, DOMMatrix, Object].some((e2) => t2 instanceof e2) ? B(t2) : this;
  }
  toFloat32Array(t2) {
  ...
  rotateAxisAngle(t2, e2, r2, n2) {
    if ([t2, e2, r2, n2].some((t3) => Number.isNaN(+t3)))
  ...
  if (!y2.isIdentity) {
    ...
    return w2.map((t3) => {
      switch (M2 = t3.c, d2 = t3.s, M2) {
        ...
        default:
          for (n2 = 1, i2 = d2.length; n2 < i2; n2 += 2)
            [c2, u2] = it(y2, [+d2[n2], +d2[n2 + 1]], x2), d2[n2] = c2, d2[n2 + 1] = u2;
          return d2;
      }
    });
  }
...
function xt(t2) {
  if (!t2)
    return { x: 0, y: 0, width: 0, height: 0, x2: 0, y2: 0, cx: 0, cy: 0, cz: 0 };
...
var dt = class {
  constructor(t2, e2) {
    ...
    if (n2 || !t2.length)
      throw TypeError(`${c}: "pathValue" is ${n2 ? "undefined" : "empty"}`);
    const s2 = M(t2);
    if ("string" == typeof s2)
      throw TypeError(s2);
    this.segments = s2;
    ...
    if ("auto" === h2) {
      ...
      Number.isInteger(h2) || "off" === h2 ? y2 = h2 : { round: y2 } = a;
  ...
  transform(t2) {
    if (!t2 || "object" != typeof t2 || "object" == typeof t2 && !["translate", "rotate", "skew", "scale"].some((e3) => e3 in t2))
      return this;
    const e2 = {};
...
function Mt(t2, e2) {
  ...
  if (i2 <= 0 || !e2 || !Number.isFinite(e2))
    return { segment: m2, index: 0, length: o2, point: l2, lengthAtSegment: a2 };
  if (e2 >= s2)
    return n2 = r2.slice(0, -1), a2 = pt(n2), o2 = s2 - a2, { segment: r2[i2], index: i2, length: o2, lengthAtSegment: a2 };
  const h2 = [];
  for (; i2 > 0; )
    m2 = n2[i2], n2 = n2.slice(0, -1), a2 = pt(n2), o2 = s2 - a2, s2 = a2, h2.push({ segment: m2, index: i2, length: o2, lengthAtSegment: a2 }), i2 -= 1;
...
function wt(t2, e2) {
  ...
  for (let t3 = 0; t3 <= s2; t3 += u2)
    a2 = gt(n2, t3), l2 = i2(a2), l2 < f2 && (o2 = a2, h2 = t3, f2 = l2);
  u2 /= 2;
  let y2 = 0, x2 = 0, p2 = 0, g2 = 0;
  for (; u2 > 0.5; )
    y2 = h2 - u2, m2 = gt(n2, y2), p2 = i2(m2), x2 = h2 + u2, c2 = gt(n2, x2), g2 = i2(c2), y2 >= 0 && p2 < f2 ? (o2 = m2, h2 = y2, f2 = p2) : x2 <= s2 && g2 < f2 ? (o2 = c2, h2 = x2, f2 = g2) : u2 /= 2;
  const d2 = Mt(r2, h2);
...
var Nt = { CSSMatrix: st, parsePathString: M, isPathArray: b, isCurveArray: S, isAbsoluteArray: w, isRelativeArray: A, isNormalizedArray: P, isValidPath: vt, pathToAbsolute: v, pathToRelative: N, pathToCurve: Z, pathToString: R, getDrawDirection: function(t2) {
  ...
  if (n2 && !r2.some((t3) => n2 === t3))
    throw TypeError(`${c}: "${n2}" is not SVGElement`);
  ...
  return "circle" === i2 ? u2 = R(function(t3) {
    ...
    let n3 = 0;
    for (; n3 < r3.length; )
      e3.push([n3 ? "L" : "M", r3[n3], r3[n3 + 1]]), n3 += 2;
...
var Ct = (t2) => null == t2;
var Pt = (a2) => {
  const { width: o2, height: m2, cx: c2, cy: u2, rx: l2, ry: h2, startOffset: f2, reversed: y2, text: x2, svgProps: p2, ellipseProps: g2, textPathProps: d2, textProps: b2, tspanProps: M2 } = a2, [w2, v2] = (0, import_react.useState)(false), [A2] = (0, import_react.useState)(`ellipse-id${(0, import_react.useId)().replaceAll(":", "-")}`), N2 = (0, import_react.useRef)();
  if ((0, import_react.useEffect)(() => {
    if (N2.current) {
      const t2 = { id: A2, type: "ellipse", rx: l2, ry: h2, cx: c2, cy: u2, style: "fill: none;", ...g2 }, e2 = N2.current, r2 = dt.shapeToPath(t2, true), n2 = document.getElementById(A2);
  ...
  }, [N2.current, y2, o2, m2, p2, c2, u2, l2, h2, g2]), Ct(o2))
  ...
  const C2 = JSON.stringify({ width: o2, height: m2, cx: c2, cy: u2, rx: l2, ry: h2, startOffset: f2, reversed: y2, text: x2, svgProps: p2, ellipseProps: g2, textPathProps: d2, textProps: b2, tspanProps: M2, rendered: w2 });
  return import_react.default.createElement("svg", i({ ref: N2, height: m2, width: o2 }, p2), import_react.default.createElement("text", i({ key: C2 }, b2), import_react.default.createElement("textPath", i({ xlinkHref: `#${A2}`, startOffset: f2 }, d2), import_react.default.createElement("tspan", M2, x2))));
};
export {
